...
- 「アウトな発言」区間だけテロップを伏字にし、音声をピー音に差し替える自主規制機能
- 任意でBGMをミックス
  ...
  "entries": [
    {
      ...
      "captions": [
        ...
        { "text": "ここは伏字になる", "start": 4, "end": 5, "ng": true }
      ]
    }
  ],
  "ngBeep": { "frequency": 1000, "volume": 1.0 }  // 省略可
}
```
...
### 「アウトな発言」の自主規制(`captions[].ng`)

差別的発言・誹謗中傷・過度に下品な発言など、そのまま公開すると規約違反や
炎上につながりかねない発言がクリップに含まれる場合、該当するテロップに
`"ng": true` を付けると、その区間だけ:

- テロップが `◯◯◯◯◯◯`(伏字。`censoredText` で文言を変更可)に置き換わり、
  赤みがかったボックスで強調表示される
- 元の音声がその区間だけ無音になり、代わりにピー音(既定1000Hz)が鳴る
  (`ngBeep.frequency` / `ngBeep.volume` で調整可能)

```jsonc
{ "text": "（ここに実際の問題発言）", "start": 4, "end": 5, "ng": true }
```

`ng:true` の区間には正確な `start`/`end`(または `entry` 側で `end`/`duration`)が
必須です。ピー音の長さをその区間に合わせて生成するためです。

なお、これは「投稿してよいかを人力で判断する」ためのポン出し用の仮処理です。
本当に問題のある発言は伏字にするのではなく、そのクリップ自体を使わない
(`entries` から外す)判断も検討してください。
