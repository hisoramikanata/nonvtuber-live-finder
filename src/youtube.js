...
export async function resolveChannel(rawInput) {
  ...
  // [\w.-] はASCII限定のため日本語ハンドル名を取りこぼす。URL区切り文字（/ ? & # 空白）
  // の手前までを丸ごと取り出し、%エンコードされていればデコードする。
  if ((m = input.match(/\/@([^/?&#\s]+)/)) || (m = input.match(/^@([^/?&#\s]+)$/))) {
    let handleName = m[1];
    try {
      handleName = decodeURIComponent(handleName);
    } catch {
      // デコードに失敗した場合は元の文字列のまま使う
    }
